...
//  Graphs
//
//  Created by Miłosz Skobejko on 18/07/2023.
//
...
private:
    int v;
    std::map<Type, std::vector<Type>> adj;

public:
    explicit graphv2(int v_) : v(v_) { }

    graphv2(std::vector<std::vector<Type> > & g)
    {
    ...
    graphv2(int v_, std::vector<std::vector<Type>>& g) : v(v_)
    {
